...
describe('Flashswap', () => {
  ...
  const WKCS = '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c';
  const USDT = '0x55d398326f99059ff775485246999027b3197955';

  beforeEach(async () => {
    const wethFactory = (await ethers.getContractAt('IWETH', WKCS)) as IWETH;
    weth = wethFactory.attach(WKCS) as IWETH;

    const fbFactory = await ethers.getContractFactory('FlashBot');
    flashBot = (await fbFactory.deploy(WKCS)) as FlashBot;
  });
  ...
  describe('flash swap arbitrage', () => {
    ...
    before(async () => {
      ...
      mdexPairAddr = await mdexFactory.getPair(WKCS, USDT);
      mdexPair = new ethers.Contract(mdexPairAddr, uniPairAbi, waffle.provider);
      pancakePairAddr = await pancakeFactory.getPair(WKCS, USDT);
    });
    ...
    it('calculate how much profit we get', async () => {
      ...
      expect(res.baseToken).to.be.eq(WKCS);
    });
